...
from coursefuzz.domain.models import GitHubImportProvenance, GitHubPullRequestDestination
    ...
    def __init__(self):
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            if path in self.files:
                ...
                return httpx.Response(200, json={"type": "file", "content": encoded, "encoding": "base64"})
            
            if path in self.dirs:
                items = self.dirs[path]
                return httpx.Response(200, json=[
                    {"name": item, "path": f"{path}/{item}", "type": "file"}
                    for item in items
                ])
...
def test_importer_success(importer, mock_client):
    assignment_json = {
        ...
        "domain_max": 10,
        "instructor_tests": [
            {"inputs": [1, 1, 1], "expected": "equilateral"}
        ]
    }
    mock_client.transport_mock.files["assignment.json"] = json.dumps(assignment_json)
    mock_client.transport_mock.files["reference.py"] = "def classify(a,b,c):\n  return 'equilateral'"
    mock_client.transport_mock.dirs["accepted"] = ["control1.py"]
    mock_client.transport_mock.files["accepted/control1.py"] = "def classify(a,b,c):\n  # control\n  return 'equilateral'"
    mock_client.transport_mock.dirs["misconceptions"] = ["wrong1.py"]
    mock_client.transport_mock.files["misconceptions/wrong1.py"] = "def classify(a,b,c):\n  return 'isosceles'"
