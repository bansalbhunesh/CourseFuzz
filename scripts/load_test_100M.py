...
def process_batch(batch_id: int, count: int) -> tuple[int, float]:
    ...
    spec = AssignmentSpec(
        ...
        accepted_solutions=(ProgramVariant(id="ctrl", title="Ctrl", source="def square(n):\n return n**2\n"),),
        mutants=(ProgramVariant(id="mut", title="Mut", misconception="Double", source="def square(n):\n return n*2\n"),),
        instructor_tests=(TestCase(inputs=(3,), expected=9, label="pos"),),
    )

    # Perform AST analysis & Matrix computation on batch
    ast_info = analyze_source_ast(spec.reference.source)
    grid = {"pos": {"mut": False}}
    diff_matrix = compute_differential_matrix(["pos"], ["mut"], grid)

    elapsed = time.monotonic() - start
    return count, elapsed

def run_100M_simulation():
    logging.info(f"Starting 100,000,000 (100 Million) Scale Simulation using {mp.cpu_count()} CPU cores...")
    ...
    with mp.Pool(processes=num_processes) as pool:
        results = pool.starmap(
            process_batch,
            [(i, BATCH_SIZE) for i in range(chunks)]
        )
    ...
    logging.info(f"100M Simulation Finished!")
