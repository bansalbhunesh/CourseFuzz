...
def run_backend_load_test(num_users: int = 500, workers: int = 25):
    logging.info(f"Starting Backend API & Serialization Load Test ({num_users} simulated user flows across {workers} workers)...")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if (successful + failed) % 100 == 0:
                    logging.info(f"API Load Test Progress: {successful + failed}/{num_users} completed...")

        elapsed = time.monotonic() - start_time
        logging.info(f"Backend API Load Test Finished in {elapsed:.2f} seconds.")
        logging.info(f"Successful API Flows: {successful}/{num_users} | Failed: {failed}/{num_users}")
