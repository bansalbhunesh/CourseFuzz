...
def run_real_data_e2e_verification():
    ...
    with tempfile.TemporaryDirectory(prefix="cf-e2e-real-") as tmp_dir:
        ...
        for json_file in scraped_files:
            ...
            logging.info(
                f"Ingested Assignment ID: {assignment_id} ({stored_assignment.spec.title})"
            )

            # Create Run
            run_view, created = run_service.create_run(
                assignment_id, idempotency_key=f"e2e-{json_file.stem}"
            )
            ...
            if analyzed_run.status == "failed":
                ...
                results_summary.append(
                    {"file": json_file.name, "status": "FAILED", "error": analyzed_run.error}
                )
                continue
                ...
                results_summary.append(
                    {
                        "file": json_file.name,
                        "status": "NO_ACTION",
                        "reason": "No surviving mutants or unverified hypotheses",
                    }
                )
                continue

            logging.info(
                f"Initial Mutation Score: {analysis.before.mutation_score}% ({analysis.before.surviving_mutants} survivors)"
            )
            ...
            if analysis.candidate:
                ...
                verified_metrics = engine.verify_applied_patch(
                    stored_assignment.spec, analysis.candidate
                )
        ...
        for res in results_summary:
            print(
                f"- [{res['status']}] {res['file']}: Before={res.get('score_before', 'N/A')}%, After={res.get('score_after', 'N/A')}%, Candidate={res.get('has_candidate', False)}"
            )
